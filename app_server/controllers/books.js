    ...
    axios
        ...
        .then((answer) => {
            if (answer.data.books) {
                answer.data.books.map((book) => {
                    book.buyPrice = parseFloat(book.buyPrice).toFixed(2);
                    book.rentPrice = parseFloat(book.rentPrice).toFixed(2);
                    return book;
                });
            }
            showContent(req, res, answer.data, "");
        })
        .catch((error) => {
...
const contentsSearch = (req, res) => {
    let search =
        req.params.search && req.params.search !== "/" ? req.params.search : "";
    search = search === "noFilter" ? "" : search;
    axios
        .get("/api/books")
        .then((answer) => {
            if (answer.data.books) {
            ...
            showContent(req, res, answer.data, search);
        })
        .catch((error) => {
            showError(req, res, error);
        });
...
const myBooks = (req, res) => {
    axios
        .get("/api/users/" + req.params.idUser + "/books")
        .then((answer) => {
            showMyBooks(req, res, answer.data);
        })
        .catch((error) => {
            showError(req, res, error);
        });
...
const deleteMyBook = (req, res) => {
    ...
    axios
        ...
        .then((answer) => {
            res.redirect("/books/myBooks/" + idUser);
        })
        .catch((error) => {
            showError(req, res, error);
        });
...
const deleteBookAdmin = (req, res) => {
    idUser = req.params.idUser;
    bookId = req.params.bookId;
    axios
        .delete("/api/users/" + idUser + "/books/" + bookId)
        .then((answer) => {
            res.redirect("/books");
        })
        .catch((error) => {
...
const showError = (req, res, e) => {
    ...
    let status = e.response ? e.response.status : e.response;
    res.render("error", {
        title: title,
        message: message,
        error: e,
...
const showContent = (req, res, data, search) => {
    res.render("searchContents", {
        title: "Bookstor.",
        logged: {
        ...
        bonusScripts:
            '<script src="../../javascripts/searchContents.js"></script>',
        search: search,
        filters: {
            genres: [
                "Action & Adventure",
...
const bookDetails = (req, res) => {
    axios
        .get("/api/books/" + req.params.idBook)
        .then((answer) => {
            axios
                .get("/api/users/" + answer.data.sellerId)
                .then((answer2) => {
                    showBookDetails(req, res, answer.data, answer2.data.user);
                })
                .catch((error) => {
                    showError(req, res, error);
                });
...
const showBookDetails = (req, res, podrobnostiKnjige, userData) => {
    profImg = "../" + req.cookies.img;
    res.render("bookDetails", {
        title: "Bookstor.",
        bonusScripts:
            '<script src="../../javascripts/bookDetails.js"></script>',
        logged: {
            name: req.cookies.name,
            img: profImg,
            id: req.cookies.id,
        ...
        bookTitle: podrobnostiKnjige.title,
        author: podrobnostiKnjige.author,
        bookReleaseYear: podrobnostiKnjige.bookReleaseYear,
        description: podrobnostiKnjige.description,
        genre: podrobnostiKnjige.genres,
        condition: podrobnostiKnjige.condition,
        buy: podrobnostiKnjige.buy,
        rent: podrobnostiKnjige.rent,
        buyPrice: podrobnostiKnjige.buyPrice,
        rentPrice: podrobnostiKnjige.rentPrice,
        location: podrobnostiKnjige.location,
        phone: podrobnostiKnjige.phone,
        timesGivenForRent: 0,
        sellerName: podrobnostiKnjige.sellerName,
        sellerId: podrobnostiKnjige.sellerId,
        sellerEmail: userData.email,
        buyerName: req.cookies.name,
        pictures: podrobnostiKnjige.pictures,
    });
};

const showMyBooks = (req, res, data) => {
    res.render("myBooks", {
        title: "Bookstor. - My Sales",
        bonusScripts: '<script src="../../javascripts/myBooks.js"></script>',
        books: data.books,
        logged: {
            name: req.cookies.name,
            img: "../" + req.cookies.img,
...
const addEditBook = (req, res) => {
    addEditBookShow(req, res);
};

const addEditBookShow = (req, res) => {
    res.render("addEditBook", {
        title: "Bookstor.",
        bonusScripts: '<script src="../javascripts/addEditBook.js"></script>',
        logged: {
            name: req.cookies.name,
            img: req.cookies.img,
            id: req.cookies.id,
        ...
        image: "../public/images/book.png",
        bookTitle: "Harry Peder",
        author: "Greta Turnberg",
        bookReleaseYear: 2020,
        description:
            "Prodam feministično knjigo Grete Turnberg, malo rabljena.",
...
const postBook = (req, res) => {
    ...
    axios({
        method: "post",
        url: "/api/users/" + idUser + "/books",
        data: {
            title: req.body.title,
            author: req.body.author,
            bookReleaseYear: req.body.year,
            description: req.body.description,
            genres: req.body.genre,
            condition: req.body.condition,
            buy: buyable,
            rent: rentable,
            buyPrice: req.body.price1,
            rentPrice: req.body.price2,
    ...
    axios
        ...
        .then((answer) => {
            axios
                .get("/api/users/" + answer.data.sellerId)
                .then((answer2) => {
                    editBookShow(req, res, answer.data, answer2.data.user);
                })
                .catch((error) => {
                    showError(req, res, error);
                });
...
const editBookShow = (req, res, podrobnostiKnjige, userData) => {
    let idBook = req.params.idBook;
    res.render("editBook", {
        title: "Bookstor.",
        bonusScripts: '<script src="../../javascripts/editBook.js"></script>',
        logged: {
            name: req.cookies.name,
            img: "../" + req.cookies.img,
            id: req.cookies.id,
        ...
        bookReleaseYear: podrobnostiKnjige.bookReleaseYear,
        description: podrobnostiKnjige.description,
        genre: podrobnostiKnjige.genres[0],
        condition: podrobnostiKnjige.condition,
        buy: podrobnostiKnjige.buy,
        rent: podrobnostiKnjige.rent,
        buyPrice: podrobnostiKnjige.buyPrice,
        ...
        phone: podrobnostiKnjige.phone,
        pictures: podrobnostiKnjige.pictures,
        picture0: podrobnostiKnjige.pictures[0],
        picture1: podrobnostiKnjige.pictures[1],
        picture2: podrobnostiKnjige.pictures[2],
        picture3: podrobnostiKnjige.pictures[3],
        picture4: podrobnostiKnjige.pictures[4],
    });
};
...
const postEditBook = (req, res) => {
    ...
    axios({
        method: "put",
        url: "/api/users/" + idUser + "/books/" + idBook,
        data: {
            title: req.body.title,
            author: req.body.author,
            bookReleaseYear: req.body.year,
